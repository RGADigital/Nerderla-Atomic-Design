...
function copyFromComponentTemplate(type, component) {
  ...
  files.forEach((file) => {
    ...
    fileContent = fileContent.replace(/FunctionComponent/g, 'ReactTypeDefinition');
    fileContent = fileContent.replace(/ComponentLowercase/g, component.toLowerCase());
    fileContent = fileContent.replace(/Component/g, component);
    fileContent = fileContent.replace(/ReactTypeDefinition/g, 'FunctionComponent');
  ...
  filesStories.forEach((file) => {
    ...
    fs.writeFileSync(destination, fileContent, 'utf-8');
    console.log(path.join('./src/stories/', type, component, file.replace(/Component/g, component)));
  });
  console.log('Done.');
}
